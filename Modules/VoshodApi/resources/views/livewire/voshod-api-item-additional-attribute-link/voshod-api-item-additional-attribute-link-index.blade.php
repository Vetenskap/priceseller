    ...
    <x-blocks.main-block>
        <flux:card class="space-y-6">
            ...
            <flux:select variant="combobox" placeholder="Выберите поле из API" label="Поле из API" wire:model="form.link">
                @foreach(\Modules\VoshodApi\HttpClient\Resources\Item::ATTRIBUTES as $attribute)
                ...
                <flux:rows>
                    @foreach($voshodApi->itemAdditionalAttributeLinks as $link)
                        <flux:row :key="$link->getKey()">
                            ...
                            <flux:cell>{{collect(\Modules\VoshodApi\HttpClient\Resources\Item::ATTRIBUTES)->firstWhere('name', $link->link)['label']}}</flux:cell>
                            <flux:cell>
